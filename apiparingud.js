...
async function listiraamatud() {
    ...
    for (var raamat of responseData.raamatud) {
        resultElement.innerHTML += '<a href="https://webapppractice12-daduf6h5hxfvhbfj.northeurope-01.azurewebsites.net/' + raamat + '"  download="' + raamat + '.txt" >' + raamat + "</a> " +
            '<a href="#" onclick="deleteObject(\'webapppractice12-daduf6h5hxfvhbfj.northeurope-01.azurewebsites.net/raamatud/' + raamat + '\')" > [kustuta]</a>' +
            "<br />";
    }
}
...
function handleResponse(form, responseData) {
    ...
    if (form.id === "raamatust-sone") {
        var output_raamatust = "Sõne " + responseData.sone + " leiti järgmistest raamatutest:  <br/>"
        var raamatuId = document.getElementById("raamatu_id").value;

        for (var tulemus of responseData.tulemused) {
            console.log(tulemus)
            if (tulemus.raamatu_id == raamatuId)
                output_raamatust += "Raamat " + tulemus.raamatu_id + " - " + tulemus.leitud + " korda! <br/>";
        }

        resultElement.innerHTML = output_raamatust
    }
}